...
public class LikeService {
    ...
    //点赞
    public void giveLike(int entityType, int entityId, int userId, int entityUserId) {
...
//        }
        redisTemplate.execute(new SessionCallback() {
            ...
            public Object execute(RedisOperations operations) throws DataAccessException {
                ...
                boolean isLike = operations.opsForSet().isMember(likeKey, userId);
                operations.multi();
                if (isLike) {
    ...
    public int findEntityLikeStatus(int entityType, int entityId, int userId) {
        String likeKey = RedisKeyUtil.getLikeKey(entityType, entityId);
        return redisTemplate.opsForSet().isMember(likeKey, userId) == true ? 1 : 0;
    }
